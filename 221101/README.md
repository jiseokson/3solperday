...
문제를 풀면서 부족했던 점: 느낌은 알겠는데 마지막 순간까지 <U>정확한 부분문제 정의를 내리지 않음</U>. 첫 번째 풀이 시도에서도 문제의 그래프가 <U>트리를 이룬다는 사실을 알았지만 알고있기만 했음</U>.(해결에 어떤 식으로 이용할 수 있을지에 대해 고민하지 않음) 그래프+dp가 적용되는 문제 거의 처음 풀어봄.(<U>문제 많이 안풀어봄</U>)  
